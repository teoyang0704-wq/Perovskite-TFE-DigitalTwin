# -*- coding: utf-8 -*-
"""Runtime evaluator (repo-safe paths). Corrected calibration (50 nm anchor)."""
...
def load_params(path=None):
    if path is None:
        ...
        for _c in (os.path.join(_here, "..", "data", "engine_parameters_runtime.csv"),
                   "data/engine_parameters_runtime.csv"):
            if os.path.exists(_c):
                path = _c; break
        else:
            raise FileNotFoundError("engine_parameters_runtime.csv not found in repo")
    P = {}
...
def evaluate(x):
    ...
    dur = durability_years(d_in, Tpk - Tng)
...
def run_grid():
    ...
    return np.array(xs), F, K, pareto_mask(F)
